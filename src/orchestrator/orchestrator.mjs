...
`;
  ...
  if (isIntegration) {
    const moduleWorkIds = (workItem.dependsOn ?? []).filter((id) => id !== "work.integration");
...
YOUR SPECIFIC TASK: Create a server entry point that imports and mounts all module routers.

Steps:
1. Read design-pack.json from the run directory to find all module responsibility units and their owned paths.
2. Read each module's contract files to understand what they export (routers, handlers, middleware).
3. Create app.js or index.js at the project root (or src/app.js / src/index.js) that wires all modules together.
4. The entry point must: import all module routers, mount them at the correct paths, configure any shared middleware, and export or start the server.
5. The file must be loadable without crashing (node --check <file> must pass).

Module work items this integration depends on:
${moduleWorkIds.length > 0 ? moduleWorkIds.map((id) => `- ${id}`).join("\n") : "- (none listed)"}
...
- You MUST create at least one of: app.js, index.js, server.js, src/app.js, src/index.js, src/server.js, src/app.mjs, or src/index.mjs.
- Do not add fallback behavior outside declared contracts.
- If a module does not yet have a router or export, create a stub that satisfies the contract surface so the entry point compiles.
