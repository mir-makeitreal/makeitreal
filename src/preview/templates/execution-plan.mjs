...
export function taskDagMermaid(dossier = {}) {
  ...
  // Doctrine: render only the work items the LLM declared in taskDag. The
  // engine does not invent "potential parallel" nodes or "parallel opportunity"
  // edges for uncovered modules.
...
export function renderExecutionPlanSection(dossier = {}) {
  ...
  // Doctrine: parallelism is a domain decision the LLM declares via
  // taskDag.maxParallelism. The engine does not compute it from the graph.
  const declaredMaxParallelism = taskDag.maxParallelism;
  ...
  return `<section id="execution-plan" class="architecture-section">
    ...
    ${Number.isFinite(declaredMaxParallelism)
      ? `<p class="section-note"><strong>Maximum parallelism: ${escapeHtml(String(declaredMaxParallelism))} concurrent agent${declaredMaxParallelism === 1 ? "" : "s"}</strong></p>`
      : ""}
