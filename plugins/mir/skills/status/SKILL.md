...
2. Start with `makeitreal-engine status "${CLAUDE_PROJECT_DIR:-$PWD}"` when the plugin binary is available.
...
4. If `dashboardRefresh.dashboardUrl` is present and dashboard auto-open is enabled, run `makeitreal-engine dashboard open "$RUN_DIR" --project-root "${CLAUDE_PROJECT_DIR:-$PWD}"` so the browser dashboard appears for the operator.
