...
Create a zero-context implementation packet before any code changes. The user-facing action is `/mir:plan`.
...
- `/mir:plan <request>` generates the Blueprint for that request and seeds pending approval.
...
When `/mir:plan` is invoked without a request, clarification is not optional. Use `AskUserQuestion` to collect the missing feature request before calling the MCP tool. Continue with one focused `AskUserQuestion` at a time until the request is specific enough to name intended behavior, module boundary, contract/API/IO expectation, and verification expectation.
...
## Test Scaffolds Are LLM-Generated

Test scaffolds are generated by Claude (the LLM), not the engine. After blueprint approval, the launch flow will generate test prompts for each contract. Claude implements the actual tests.
...
- `/mir:plan <request>` may be the first Make It Real command in a project. Submitting through `mir_blueprint` creates `.makeitreal/runs/<runSlug>/`, records the current run, and writes artifacts under that run directory.
