...
Prepare the project so Make It Real can run as a Claude Code harness. Keep hook registration and engine details internal; the user-facing action is `/mir:setup`.

Setup is optional for ordinary new work. `/mir:plan <request>` creates `.makeitreal/runs/...`, selects the current run, and writes the ignore entry automatically. Use setup when the operator wants a first-run diagnostic/config bootstrap or wants to select an existing run with `--run`.
...
5. Confirm plugin-native hooks are available from `hooks/hooks.json`, or use the engine's manual hook setup only when the plugin is not loaded.
...
3. Make It Real asks an LLM judge to classify the reply and records clear approval/rejection decisions as `makeitreal:interactive-review:llm`; if approval includes launch intent, continue to `/mir:launch`
4. `/mir:status`

`/mir:setup` may be run before that flow, but it is not a required gate for ordinary planning.
...
Use `/mir:config` only when the user wants to change optional feature flags such as live wiki sync.
