...
function softwareScenarios({ designPack }) {
  return (designPack.sequences ?? []).filter((seq) => !harnessSequence(seq));
}
...
function modelReviewDecisions({ blueprintReview }) {
  return reviewDecisionSummary(blueprintReview);
}
...
export function buildSystemDossier({
  ...
  return {
    ...
    designPatterns: designPack.designPatterns ?? [],
