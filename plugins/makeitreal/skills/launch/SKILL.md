...
- For all real Claude Code execution, use the parent-session native Task path:
  1. Read status first. When `boardStatus.recommendedNativeTaskConcurrency` is present, use that value unless the operator explicitly requested a smaller concurrency. The recommendation is derived from unblocked modules.
  ...
  5. Aggregate their JSON reports and record them with `makeitreal-engine orchestrator native finish "$RUN_DIR" --work "$WORK_ITEM_ID" --attempt "$ATTEMPT_ID" --result-stdin`. The canonical stdin envelope is one node report (`makeitrealReport`, `makeitrealPmReport`, or `makeitrealEvidenceReport`) plus `makeitrealReviews: [{...}]`; if a native reviewer returned `{ "makeitrealReview": {...} }`, unwrap that object into the `makeitrealReviews` array. Validate each Task result before `native finish`. Record valid sibling envelopes immediately, then rerun only invalid scoped Tasks once with the same handoff packet. Do not call `native finish` with empty stdin, prose-only output, or missing node report JSON. If a scoped Task still cannot produce the report, leave only that work item Running and report the protocol blocker without mutating its board state.
