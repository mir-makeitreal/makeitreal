...
2. If this repository's engine is available, use `dev-harness/bin/harness.mjs` as the internal policy engine.
3. Record the active run with `makeitreal-engine setup "$CLAUDE_PROJECT_DIR" --run <runDir>`.
4. Confirm plugin-native hooks are available from `hooks/hooks.json`, or use the engine's fallback hook setup only when the plugin is not loaded.
5. Do not present `hooks install` or hook file paths as normal workflow commands.
...
3. Review the generated Blueprint preview and answer naturally in the conversation
4. Make It Real asks an LLM judge to classify the reply and records clear approval/rejection decisions as `makeitreal:interactive-review:llm`; if approval includes launch intent, continue to `/makeitreal:launch`
5. `/makeitreal:status`
