...
| Controller provides full task context | Engine writes `agent-handoff.json` and prompt from PRD slice, Blueprint work item, contracts, dependency artifacts, allowed paths, and verification command. | Handoff prompt is passed through Claude Code native subagent invocation; there is no separate headless Claude runner path. |
