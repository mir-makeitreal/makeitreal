...
| Agent role catalog | GSD agent roster | Small role set: `repo-mapper`, `boundary-architect`, `contract-reviewer`, `implementation-worker`, `verification-reviewer`, `rework-planner`. | Launch and plan docs can select roles without broad context dumping; tests assert role prompts stay scoped. |
| Execution waves | GSD execute-phase | Board dependency graph projects wave 1/2/3 and launch reports wave state. | Dashboard and status show wave grouping; orchestrator dispatch respects wave boundaries. |
...
| Scoped execution | Yes | Needs first-class role catalog and native-agent-friendly routing. | P0 |
...
3. Agent role catalog and wave projection.
4. Progress-next and resume packet.
5. Verification debt and rework plan generator.
6. Runner/model profiles.
7. Read-only ship summary.
8. Capability pack skeleton.
